...
* 엔드 투 엔드 테스트
  ...
  * 모든 외부 애플리케이션을 포함해 시스템을 최종 사용자의 관점에서 검증하는 테스트
  * 동의어: UI 테스트, GUI 테스트, 기능 테스트
  * 데이터베이스, 파일 시스템, 결제 게이트 웨이 라는 세 가지 프로세스 외부 의존성으로 작동하는 애플리케이션을 예로 들면,
    * 데이터베이스, 파일 시스템 : 통합 테스트에 포함 (완전히 제어 가능하므로)
    * 결제 게이트웨이 : 테스트 대역으로 대체 (제어할 수 없으므로)
  * 유지 보수 측면에서 비용이 가장 많이 들기 때문에 모든 단위 테스트, 통합 테스트 통과 후 실행하는 것이 좋다. 개발자 로컬이 아닌 빌드 서버에서만 실행하는 것도 방법이다.
  * 엔드 투 엔드 테스트도 여러 이유로 프로세스 외부 의존성을 테스트 대역으로 대체해야 하는 경우가 있다. 따라서 통합 테스트와의 사이에 뚜렷한 경계가 다.
